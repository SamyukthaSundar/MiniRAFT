...
const QUORUM                  = Math.floor((1 + state.peers.length + 1) / 2);
// For a 3-node cluster: total = 3, QUORUM = floor(3/2)+1 = 2. ✓
...
async function _startElection() {
  ...
  console.log(`[${state.replicaId}] 🗳  Election — term ${electionTerm}, need ${QUORUM} votes`);
...
function _becomeLeader(term) {
  ...
  console.log(`[${state.replicaId}] ★ Became LEADER for term ${term}`);
